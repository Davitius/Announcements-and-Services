...
$servantuser = $author = $row->user;
$feedbackmail = $_COOKIE['loginuser'];
$servant = "SELECT * FROM users WHERE usermail = '$feedbackmail'";
$servantresult = $mysql->query($servant);
$servantrow = $servantresult->fetch_object();
...
$query = "UPDATE `services` SET `counter`='".$counter."' WHERE `id` = '".$statement_id."'";
...
<div class="row">
      ...
      <div class="carousel col-lg-12" style="padding:0; margin:0;">
...
<?php
    ...
    <a class="" href="userinfo.php?id=<?php echo $auth->id ?>"><img src="data:image/jpeg;base64, <?php echo $show_img ?>" alt="" width="215" height="215" class="rounded-circle"></a>
           <?php else: ?>
    <a class="" href="userinfo.php?id=<?php echo $auth->id ?>"><img alt="" width="215" height="215" class="rounded-circle" src="img/usericon.png"></a>
            <?php endif; ?>
            <br> <br>
...
<img class="ratingstar" src="img/ystar.PNG">  <?php endif; ?>
        ...
        <div class="panel-group" id="collapse-group">
                        <div class="btn btn-light" style="backgroundcolor: white;">
...
<?php
    $minimap=$row->map;
    $minimap=str_replace('width="600"', 'width="280"', $minimap);
?>
<center><p class=""><?php echo $minimap; ?></p></center>
...
<?php
  ...
  <?php  if($key2 == $usercookie): ?>
   ...
   <label class="" style="color: aliceblue">თქვენ უკვე შეაფასეთ.</label>
  <?php goto label2; ?>
...
<!-- შეფასებების და კომენტარების დამატება -->
<?php
if(isset($_POST['addfeedback'])){

$serviceid = $statement_id;

 $feedb_name = $servantrow->name;
 $feedb_lname = $servantrow->lname;
...
$date = date ("d.m.y");
    ...
    if($feedbackstar == "1"){
    $query = "UPDATE `users` SET `star1`='".$star1."' WHERE `usermail` = '".$servantuser."'";
    $query_run = mysqli_query($connect, $query) or die(mysqli_error());
    }
    if($feedbackstar == "2"){
    $query = "UPDATE `users` SET `star2`='".$star2."' WHERE `usermail` = '".$servantuser."'";
    $query_run = mysqli_query($connect, $query) or die(mysqli_error());
    }
    if($feedbackstar == "3"){
    $query = "UPDATE `users` SET `star3`='".$star3."' WHERE `usermail` = '".$servantuser."'";
    $query_run = mysqli_query($connect, $query) or die(mysqli_error());
    }
    if($feedbackstar == "4"){
    $query = "UPDATE `users` SET `star4`='".$star4."' WHERE `usermail` = '".$servantuser."'";
    $query_run = mysqli_query($connect, $query) or die(mysqli_error());
    }
    if($feedbackstar == "5"){
    $query = "UPDATE `users` SET `star5`='".$star5."' WHERE `usermail` = '".$servantuser."'";
    $query_run = mysqli_query($connect, $query) or die(mysqli_error());
    }

    $mysql->close();

    echo 'შეფასება დაემატა' . '<br>' . $serviceid. '<br>' . $servantuser. '<br>' . $comment. '<br>' . $feedbackstar. '<br>' . $feedbackmail. '<br>' . $date;
